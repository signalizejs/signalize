...
export default async ({ $, options }) => {
	const { attributePrefix, attributeSeparator } = $;
	const { on, scope } = $.import(['on', 'scope']);
	...
	const renderedTemplateStartComment = options?.prerenderedBlockStart ?? 'prerendered';
	const renderedTemplateEndComment = options?.prerenderedBlockEnd ?? '/prerendered';
	let inited = false;
